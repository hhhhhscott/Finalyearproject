    ...
    for iter in range(maxiter):
        ...
        with warnings.catch_warnings():
            ...
            with open('out.log', 'w+') as f:
                ...
                stream = sys.stdout
                sys.stdout = f
                prob.solve(solver=cp.SCS, verbose=False, scale=1e-8, max_iters=250)
                sys.stdout.flush()
                sys.stdout = stream
    ...
    for iter in range(maxiter):
        ...
        with warnings.catch_warnings():
            ...
            with open('out.log', 'w+') as f:
                ...
                sys.stdout.flush()
                sys.stdout = stream
        print("DC_RIS_prob.value:", prob.value)
        if verbose:
...
def DC_RIS(libopt, h_d, G, verbose):
    ...
    epsilon2 = 1e-8
    #    obj_DC_RIS =0
    F_DC_RIS, theta_DC_RIS = DC_main(N, M, L, h_d, G, maxiter, iter_num, epsilon, verbose,
                                     epsilon2)  # 这里把参数传进去之后就用论文24那套字母了
