...
def learning_flow(libopt, Noiseless, Proposed, DCfull, NoRIS, NoDS, SVD, h_d, G, dic):
    x_store = dic['x_store']
    f_store = dic['f_store']
    theta_store = dic['theta_store']

    x_DC_full = dic['x_DC_full']
    f_DC_full = dic['f_DC_full']
    theta_DC_full = dic['theta_DC_full']
    ...
    if DCfull:
        print('DC full case is running')

        theta_DC = theta_DC_full

        h3 = np.zeros([libopt.N, libopt.M], dtype=complex)
        for i in range(libopt.M):
            h3[:, i] = h_d[:, i] + G[:, :, i] @ theta_DC
        w_glob = copy.deepcopy(w_0)
        net_glob.load_state_dict(w_glob)

        idxs_users = np.asarray(range(libopt.M))
        idxs_users = idxs_users[x_DC_full[:, 0] == 1]
        loss_train_DC, accuracy_test_DC, loss_test_DC = Learning_iter(libopt,d,net_glob,w_glob,idxs_users,
                                                                      train_labels,train_labels,test_images,test_labels,1,
                                                                      x_DC_full[libopt.Jmax],f_DC_full[:,libopt.Jmax],h3)
        result['loss_train_DC']=np.asarray(loss_train_DC)
        result['accuracy_test_DC']=np.asarray(accuracy_test_DC)
        result['loss_test_DC']=np.asarray(loss_test_DC)

        print('result {}'.format(result['accuracy_test_DC'][len(result['accuracy_test_DC']) - 1]))
    ...
    if Proposed:
        ...
        for ib in range(len(libopt.bit)):
            ...
            loss_train1, accuracy_test1, loss_test1 = Learning_iter(libopt, d, net_glob, w_glob, idxs_users,
                                                                    ...
                                                                    test_labels, 1,
                                                                    x_store[libopt.Jmax], f_store[:, libopt.Jmax],
                                                                    h[:, :, ib]
                                                                    )
