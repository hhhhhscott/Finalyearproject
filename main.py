...
def initial():
    ...
    libopt.add_argument('--M', type=int, default=20, help='total # of devices')  # 有m个客户端
    libopt.add_argument('--N', type=int, default=1, help='# of BS antennas')  # PS天线数量（PS?）
    libopt.add_argument('--L', type=int, default=20, help='RIS Size')  # 反射界面的相变器件数量
    ...
    libopt.add_argument('--trial', type=int, default=30, help='# of Monte Carlo Trials')  # 蒙特卡洛试验次数
    ...
    libopt.add_argument('--epochs', type=int, default=200, help="rounds of training,T")
...
if __name__ == '__main__':
    ...
    DC_full_set = np.ones([libopt.trial, ]) * np.nan
    DC_NORIS_set = np.ones([libopt.trial, ]) * np.nan
    DC_NODS_set = np.ones([libopt.trial, ]) * np.nan
    DG_NORIS = np.ones([libopt.trial, ]) * np.nan  # SVD算法用的，不需要
    ...
    for i in range(libopt.trial):
        ...
        if libopt.set == 1:
            ...
            print(sum(libopt.K))
            libopt.dx2 = np.random.rand(
                int(libopt.M - np.round(libopt.M / 2))) * libopt.range - libopt.range  # [-range,0]
            # setting 1 region II
        else:
        ...
        if DC_full:
            ...
            print('Running full DC algorithm')
            gamma = [15]
            [x_DC_full, obj_DC_full, f_DC_full, theta_DC_full] = DC_DS_RIS_2.DC_DS_RIS_2(libopt=libopt, h_d=h_d, G=G,
                                                                                         x0=x, gamma_set=gamma,
                                                                                         verbose=libopt.verbose)
            end = time.time()
            print("Running time: {} seconds".format(end - start))

            DC_full_set[i] = obj_DC_full

        else:
            [x_DC_full, obj_DC_full, f_DC_full, theta_DC_full] = [0, 0, 0, 0]
            pass
        ...
        # print('Proposed_a2:{a} Proposed_a1:{b} DC_full_a2:{dca2} DC_full_a1:{dca1} NoRIS:{c} NoDS:{d} DG:{e}'.format(
        #     a=obj_new[libopt.Jmax],
        #     b=obj_new[0],
        #     dca2=obj_DC_full[libopt.Jmax],
        #     dca1=obj_DC_full[0],
        #     c=obj_new_NORIS[0],
        #     d=obj_DC_RIS,
        #     e=obj_SVD))

        # TODO：第一步，控制其他参数一样的情况下，研究各方法iteration和准确率的关系（full batch）
        # TODO: Setting_1 // Setting_2
        # TODO: full-batch // mini-batch
        # TODO: (0)无误差信道传输
        # TODO：(1)Proposed保留，令f为1*1
        # TODO: (2)DC轮换优化，带RIS与Device Selection
        # TODO: (3)DC轮换优化，不带RIS
        # TODO: (4)DC轮换优化，不带Device Selection

        # TODO：第二步，控制iteration相同的情况下，研究各方法L和准确率的关系（full batch）
        # TODO: Setting_1 // Setting_2
        # TODO:  full-batch // mini-batch
        # TODO: (0)无误差信道传输
        # TODO：(1)Proposed保留，令f为1*1
        ...
        # TODO: 第三步，探究theta离散值和连续值对准确率的影响
        # TODO: Setting 1 // Setting 2
        # TODO: theta分割为1，2，3，\infty
        # TODO: (0)无误差传输
        # TODO: (1)DC轮换优化，带RIS与Device Selection
        ...
        dic['x_DC_full'] = copy.deepcopy(x_DC_full)
        dic['f_DC_full'] = copy.deepcopy(f_DC_full)
        dic['theta_DC_full'] = copy.deepcopy(theta_DC_full)
        ...
        result, _ = flow.learning_flow(libopt, Noiseless, Proposed, DC_full, NoRIS, NoDS, SVD,
                                       h_d, G, dic)
        end = time.time()
        print("Running time: {} seconds".format(end - start))
    ...
    #        end = time.time()
    #        print("Running time: {} seconds".format(end - start))
    np.savez(filename, vars(libopt), result_set, SCA_Gibbs, DC_full_set, DC_NORIS_set, DC_NODS_set)
    # 文件名，libopt变量列表，各个方法的（训练集损失，测试集准确率，测试集损失），propsoed(23)式结果，DC NORIS(23)式结果，DC_NODS(23)式子结果
